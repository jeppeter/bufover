...
/*define for driver name*/

#define DEVICE_NAME L"\\Device\\HelloDrv"
#define LINK_NAME L"\\DosDevices\\HelloDrv"
#define LINK_GLOBAL_NAME L"\\DosDevices\\Global\\HelloDrv"
...
{
	DEBUG_PRINT("[HelloDrv]DispatchCreate\n");
...
{
	DEBUG_PRINT("[HelloDrv]DispatchClose\n");
	pIrp->IoStatus.Status = STATUS_SUCCESS;
	pIrp->IoStatus.Information = 0;
	IoCompleteRequest(pIrp,IO_NO_INCREMENT);
	return STATUS_SUCCESS;
...
NTSTATUS DispatchIoctl(PDEVICE_OBJECT pDrvObj,PIRP pIrp)
{
	...
	DEBUG_PRINT("[HelloDrv]DispatchIoctl\n");
